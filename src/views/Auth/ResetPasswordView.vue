    ...
    <div class="relative bg-white z-1 dark:bg-gray-900 sm:p-0">
      ...
      >
        <div class="flex items-center justify-center z-1">
          <common-grid-shape />
          <div class="flex flex-col max-w-xl lg:w-md bg-white p-10 rounded-md">
...
const handleSubmit = () => {
  const payload: ResetPasswordDataInterface = {
    token: token.value,
    email: email.value,
  ...
  if (!payload.password || !payload.password_confirmation) {
    showToast({title: 'Please fill the required field!', icon: 'error'})
    return
  }

  const isPasswordConfirmed = validatePasswordConfirmation(payload.password, payload.password_confirmation)
  if (!isPasswordConfirmed) {
    showToast({title: 'Please confirm your new password!', icon: 'error'})
    return
  }
  ...
  auth
    ...
    .catch(err => {
      const errorMessage = err.response?.data?.message || 'Failed to reset your password!'
