    ...
    <div class="relative bg-white z-1 dark:bg-gray-900 sm:p-0">
      ...
      >
        <div class="flex items-center justify-center z-1">
          <common-grid-shape />
          <div class="flex flex-col max-w-xl bg-white p-10 rounded-md">
...
const handleSubmit = () => {
  const registerData: UserInterface = {
    name: name.value,
    email: email.value,
  ...
  const isDataNotValid = validateData(registerData)
  if (isDataNotValid) {
    showToast({title: 'Please fill the required fields!', icon: 'error'})
    return
  }

  const isEmailValid = validateEmail(registerData.email)
  if (!isEmailValid) {
    showToast({title: 'Please input valid email!', icon: 'error'})
    return
  }

  const isPasswordConfirm = validatePasswordConfirmation(registerData.password, registerData.password_confirmation)
  if (!isPasswordConfirm) {
    showToast({title: 'Your password & password confirmation is not match!', icon: 'error'})
    return
  }

  if (!agreeToTerms.value) {
    showToast({title: 'Please agree to terms and conditions!', icon: 'warning'})
    return
  }
  ...
  auth
    ...
    .catch(err => {
      const errorMessage = err.response?.data?.message || 'Failed to register new account'
...
const validateData = (data: UserInterface) => {
...
const validatePasswordConfirmation = (password: string, passwordConfirm: string) => {
